...
name: lark-base
version: 1.2.3
description: "飞书多维表格（Base）操作：建表、字段、记录、视图、统计、公式/lookup、表单、仪表盘、workflow、角色权限；遇到 Base/多维表格/bitable 或 /base/ 链接时使用。支持通过目录库选择文件录入目标表。文件导入转 lark-drive，认证/授权转 lark-shared。"
metadata:
  requires:
    bins: ["lark-cli"]
...
- 用户要把文件（合同、发票、采购单、客户资料、开票信息等）录入 Base / 多维表格。
...
当用户要把本地文件（合同、发票、采购单、客户资料、开票信息等）录入 Base 时，使用共享目录库定位目标表：

1. 先读取目录库：  
   `lark-cli base +record-list --base-token ENcxbFHLSayG8bs7nLIc45elnub --table-id tbl6xB2zx7XUxsq1 --as user`

2. 默认目标表是「销售配件」：  
   `base_token=AmuqbgOCkax4TtsTEfrchb5EnPc`，`table_id=tblVuMC2ginVS9nA`。  
   如果文件内容明显属于销售配件/销售合同（如合同、订单、报价、发货单），可直接使用该默认表。

3. 如果文件内容明显不属于默认表（如客户开票信息应入「客户信息」、供应商资料应入「供应商信息」、库存/采购/核销记录应入对应表），或者用户未明确指定目标表，**必须列出目录库中所有可选表格**（含序号、表名称、Base 名称、用途说明），让用户用序号或表名指定目标；不要直接猜测。

4. 用户指定目标表后，按「写入前置规则」读取该表真实字段结构，再执行录入。  
   录入时只写存储字段；附件字段走 `+record-upload-attachment`。

5. 目标表为「客户信息」或「供应商信息」时，录入前先用名称查重：
   - 客户信息表：用 `客户名称` 字段匹配
   - 供应商信息表：用 `供应商名称` 字段匹配
   若已存在同名记录，向用户报告重复并停止新建；如需更新现有记录，请用户明确说明后再执行。
